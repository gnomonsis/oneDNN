...
typename utils::enable_if<fmt_i == nhwc && fmt_o == nchw && type_i == mkldnn_bin && type_o == mkldnn_f32>::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
        const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
        int smask = attr ? attr->output_scales_.mask_ : 0;
        return (smask == 0 || smask == 2) && order_keep && 
            input_d._md->format == nhwc && output_d._md->format == nchw && input_d.data_type() == bin && output_d.data_type() == f32;
    }
    ...
    static status_t execute(const cpu_reorder_pd_t *pd,
        ...
        const memory_tracking::grantor_t &scratchpad) {
            ...
            parallel_nd(N, H, W, [&](int n, int h, int w) {
                ...
                for (int cb = 0; cb < CB; ++cb) {
                    uint8_t bin_val = 0x00;
                    ptrdiff_t in_off = cb * input_d.blocking_desc().strides[0][1];
                    for (int c = cb * nbits, shift = 0; c < std::min(C, (cb + 1) * nbits); c++, shift++) {
                        ptrdiff_t out_off = c * output_d.blocking_desc().strides[0][1];
                        o[out_off] = extract_bit(i[in_off], shift);
                    }
                }
...
typename utils::enable_if<fmt_i == nhwc && fmt_o == nchw && (type_i != mkldnn_bin || type_o != mkldnn_f32)>::type>
{
    static bool is_applicable(const memory_desc_wrapper &input_d,
        const memory_desc_wrapper &output_d, const primitive_attr_t *attr) {
        int smask = attr ? attr->output_scales_.mask_ : 0;
        return (smask == 0 || smask == 2) && order_keep && input_d._md->format == nhwc && output_d._md->format == nchw;  // add check?
    }
